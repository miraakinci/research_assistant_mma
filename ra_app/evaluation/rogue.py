...
def get_word_ngrams(n, sentences):
    ...
    for sentence in sentences:
        words.update(get_ngrams(n, split_into_words([sentence])))

    return words


def get_index_of_lcs(x, y):
    return len(x), len(y)
...
def recon_lcs(x, y):
    ...
    def recon(i, j):
        if i == 0 or j == 0:
            ...
            return recon(i - 1, j - 1) + [(x[i - 1], i)]
        elif table[i - 1, j] > table[i, j - 1]:
            return recon(i - 1, j)
        else:
            return recon(i, j - 1)

    i, j = get_index_of_lcs(x, y)
    recon_tuple = tuple(map(lambda r: r[0], recon(i, j)))
    return recon_tuple
...
def rouge_l_sentence_level(evaluated_sentences, reference_sentences):
    ...
    return f_lcs(lcs, m, n)


def union_lcs(evaluated_sentences, reference_sentence):
    """
    ...
    for eval_s in evaluated_sentences:
        evaluated_words = split_into_words([eval_s])
        lcs = set(recon_lcs(reference_words, evaluated_words))
...
def rouge_l_summary_level(evaluated_sentences, reference_sentences):
    ...
    m = len(split_into_words(reference_sentences))

    # total number of words in evaluated sentences
    n = len(split_into_words(evaluated_sentences))

    union_lcs_sum_across_all_references = 0
    for ref_s in reference_sentences:
        union_lcs_sum_across_all_references += union_lcs(evaluated_sentences, ref_s)
    return f_lcs(union_lcs_sum_across_all_references, m, n)